...
bool OrderList::insert(int position, Order obj) {
	//position validation
...
int OrderList::getNumOfItem() {
	return numOfItem;
}


bool OrderList::displayAll() {
	Node* curr;
	Order temp;
	curr = head_ptr;
	while (curr != NULL) {
...
bool OrderList::checkList(std::string orderid) {
	Order ordertemp;
	bool repeat = false;
	if (numOfItem == 0) {
		return repeat;
...
Order OrderList::getCurrNode(int position){
